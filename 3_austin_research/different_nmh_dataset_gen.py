...
def generate_dataset(model, NUM_PROMPTS, TOTAL_TYPES = 4):
    """
    apologies for the lazy coding here

    we ultimately dont use objects/names/or places, we just filter them to make sure they are all single tokens WITH spaces
    """
    objects = [
    "perfume",
    "scissors",
    ...
    # the names here, altho initialized with a space, are not guaranteed to be one token
    names = [
    " Sebastian",
    " Jack",
    " Jeremiah",
    ...
    lower_case_still_same_name = [] # names for which the lower case version of it is also a single token
    for name in names:
        lower_name = name.lower()
        if len(model.to_str_tokens(lower_name, prepend_bos=False)) == 1:
            lower_case_still_same_name.append(name)

    one_token_objects = [] # objects for which the WITH space, lower case version of it is also a single token
    for obj in objects:
        longer_obj = " " + obj
        if len(model.to_str_tokens(longer_obj, prepend_bos=False)) == 1:
            one_token_objects.append(longer_obj)

    one_token_places = [] # places for which the WITH space, lower case version of it is also a single token
    for place in places:
        longer_place = " " + place
    ...
    for name in names: # filtering out the names ig
        longer_name = name
    ...
    assert NUM_PROMPTS % TOTAL_TYPES == 0
    assert NUM_PROMPTS % 6 == 0 # sorry this is an excuse for bad coding
