...
# %%
for batch in range(1):#tqdm(range(num_batches)):
    ...
    logit_diffs, _, _, self_repair_from_heads, self_repair_from_layers, self_repair_from_LN, self_repair, ablated_cache, ablated_logits = ablate_instance_and_get_repair((ablate_layer, ablate_head), clean_tokens, corrupted_tokens, per_head_direct_effect, all_layer_direct_effect, cache, logits)
...
# %% 
last_layer_clean_neurons: Float[Tensor, "d_mlp batch pos"] = per_neuron_direct_effect[-1, :, :]
last_layer_ablated_neurons: Float[Tensor, "d_mlp batch pos"] = ablated_per_neuron_direct_effect[-1, :, :, :]
# %% 
...
# %%
hist((last_layer_ablated_neurons - last_layer_clean_neurons).mean((-1, -2)).cpu(), title = "Mean Self-Repair of neurons in last layer")
top_self_repair_neurons = topk_of_Nd_tensor((last_layer_ablated_neurons - last_layer_clean_neurons).mean((-1, -2)), 10)

print("Neurons with top self-repair:")
for neuron in top_self_repair_neurons:
    print(neuron, ":", (last_layer_ablated_neurons - last_layer_clean_neurons)[neuron].mean((-1, -2)).item())
# %%
top_self_repair = topk_of_Nd_tensor(self_repair, 30)
top_self_repair_by_last_layer = topk_of_Nd_tensor((ablated_all_layer_direct_effect - all_layer_direct_effect)[-1], 30)

#[i in top_self_repair_by_last_layer for i in top_self_repair]
# %%
instance = 5
batch = top_self_repair_by_last_layer[instance][0]
pos = top_self_repair_by_last_layer[instance][1]

x = last_layer_clean_neurons[:, batch, pos].cpu().numpy()
y = last_layer_ablated_neurons[:, batch, pos].cpu().numpy()
