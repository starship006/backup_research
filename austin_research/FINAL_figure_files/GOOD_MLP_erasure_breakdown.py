...
# %%
logit_diffs_across_everything = torch.zeros(TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
direct_effects_across_everything = torch.zeros(TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
ablated_direct_effects_across_everything = torch.zeros(TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
self_repair_from_heads_across_everything = torch.zeros(TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
self_repair_from_layers_across_everything = torch.zeros(TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
self_repair_from_LN_across_everything = torch.zeros(TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
self_repair_across_everything = torch.zeros(TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)



last_layer_clean_neurons_across_everything = torch.zeros(model.cfg.d_mlp, TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
last_layer_ablated_neurons_across_everything = torch.zeros(model.cfg.d_mlp, TOTAL_PROMPTS_TO_ITERATE_THROUGH, PROMPT_LEN - 1)
# %%
ablate_layer = 10
ablate_head = 4
for batch in tqdm(range(num_batches)):
    ...
    # store activations
    logit_diffs_across_everything[clean_batch_offset:corrupted_batch_offset, :] = logit_diffs
    direct_effects_across_everything[clean_batch_offset:corrupted_batch_offset, :] = direct_effects
    ablated_direct_effects_across_everything[clean_batch_offset:corrupted_batch_offset, :] = ablated_direct_effects
    self_repair_from_heads_across_everything[clean_batch_offset:corrupted_batch_offset, :] = self_repair_from_heads
    self_repair_from_layers_across_everything[clean_batch_offset:corrupted_batch_offset, :] = self_repair_from_layers
    self_repair_from_LN_across_everything[clean_batch_offset:corrupted_batch_offset, :] = self_repair_from_LN
    self_repair_across_everything[clean_batch_offset:corrupted_batch_offset, :] = self_repair
    
    last_layer_clean_neurons_across_everything[:, clean_batch_offset:corrupted_batch_offset, :] = last_layer_clean_neurons
    last_layer_ablated_neurons_across_everything[:, clean_batch_offset:corrupted_batch_offset, :] = last_layer_ablated_neurons
...
top_self_repair = topk_of_Nd_tensor(self_repair_across_everything, num_prompts_to_consider)
top_self_repair_by_last_layer = topk_of_Nd_tensor(self_repair_from_layers_across_everything, num_prompts_to_consider)
top_DE_in_head = topk_of_Nd_tensor(direct_effects_across_everything, num_prompts_to_consider)



neuron_self_repair = last_layer_ablated_neurons_across_everything - last_layer_clean_neurons_across_everything
...
print("Total self-repair:", self_repair_across_everything[batch, pos])
print("DE:", direct_effects_across_everything[batch, pos])


x = last_layer_clean_neurons_across_everything[:, batch, pos].cpu().numpy()
y = last_layer_ablated_neurons_across_everything[:, batch, pos].cpu().numpy()
...
fig.add_trace(go.Scatter(x=x_values, y=average_cumulative_percentages,
                         mode='lines', name='Average Cumulative Percentage'))
fig.update_layout(
    title=f"Average Cumulative Percentage of Last Layer Self-Repair explained by Top-X Neurons on Top {PERCENTILE * 100}% of L{ablate_layer}H{ablate_head} Examples",
    xaxis_title="Top X Neurons",
...
fig.write_html(FOLDER_TO_WRITE_GRAPHS_TO + f"average_cumulative_self_repair_{safe_model_name}_L{ablate_layer}H{ablate_head}_{PERCENTILE}.html")
