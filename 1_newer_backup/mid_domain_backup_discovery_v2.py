...
if False:
    # generate standard, 'off by one' corrupted prompts
    ...
    _, batch_size_clean, clean_tokens, answer_tokens_clean = generate_clean_corrupted_induction_prompts(model, gen_size = 100, prompt_half_length=12)
    _, batch_size_corrupted, corrupted_tokens, answer_tokens_corrupted = generate_clean_corrupted_induction_prompts(model, gen_size = 100, prompt_half_length=12)

    # merge into single group
    batch_size = min(batch_size_clean, batch_size_corrupted)
...
def accumulated_backup_amount(per_head_logit_diff: Float[Tensor, "layer head"], layer, head, clean_per_head_logit_diff: Float[Tensor, "layer head"]):
   """
    Given a clean and intervened per_head_logit_diff (or some other metric per head), returns the amount of backup relative to a certain head
   """
...
# %% Get Logit Contributors (Induction heads maybe?) and display all logits
top_heads = []
k = 5
...
# %% Ablate a head and see effect on other heads' logit diffs
...
# %% For each head, ablate it and see how much it gets backed up via logit diff
...
# %% Define Noising/Denoising IOI Metrics
...
# %% Path Patch into 11.3 and see results
layer = 11
head = 3
imshow(
...
# %% See prev-token-attention scores
...
def mean_ablation_hook(
    ...
    attn_result[:, :, head_index_to_ablate, :] = torch.zeros(attn_result[:, :, head_index_to_ablate, :].shape)
    return attn_result
# %% def get_backup_from_metric_and_ablations and run it on prev_attn_scores
def get_backup_from_metric_and_ablations(model, clean_tokens, clean_cache, corrupted_cache, return_item, accumulated_backup_amount, 
    ...
    """
    performs a suite of activation patches
...
# %% plot accumulated logit iff backup results for prev_attn_scores
...
# %% graph results of induction logit diff
# graph the results
x =  per_head_logit_diff.flatten()
# make subplot fig
fig = plotly.subplots.make_subplots(rows=1, cols=3, subplot_titles=("Zero Ablation", "Mean Ablation", "Sample Ablation"), shared_yaxes=True)
...
# %% look at specific head ablations to see backup of logit diffs

ablated_logit_diff, _, _ = act_patch(
            model = model,
            orig_input = clean_tokens,
            new_cache = corrupted_cache,
            patching_nodes = [Node("z", layer = 7, head = 7)],
            patching_metric = partial(calc_all_logit_diffs, use_clean_cache = True),
            verbose = False,
            apply_metric_to_cache = True
        )

imshow(ablated_logit_diff - per_head_logit_diff)
# %%
...
# %% Focusing on Prev Token Heads - Zero Path Patch, and see Change Prev Token Attention in other heads
layer = 5
head = 1
head_results = []
for i in ["q", "k", "v", "pattern"]:
...
# %% Get and plot the effect of Noisily Zero Ablating from 1.3
layer = 5
head = 1
model.reset_hooks()
temp_hook_fn = partial(zero_ablation_hook, head_index_to_ablate = head)
