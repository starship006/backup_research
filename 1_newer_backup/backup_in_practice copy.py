...
ablate_heads = [[4,2], [4,5]]#[i for i in top_heads if i[0] != 11]
...
# %%
for i in range(12):
    visualize_backup_from_top_act_diff(3,i)
...
def calculate_head_activating_other_backup_amounts(layer, head):
    ...
    # TODO, CHECK AND BRING BACK INTO FULL!!!!!
    new_cache = act_patch(model, owt_tokens, [Node("z", layer, head)], return_item, corrupted_owt_tokens, apply_metric_to_cache= True)
    ...
    for batch, pos in top_backup_locations:
        ...
        for tenth in range(11):
            backup_heads_prompt = [[i,j] for i in range(clean_version.shape[0]) 
                for j in range(clean_version.shape[1]) if (specific_example - clean_version)[i,j] > (tenth / 10)]
...
#%%
for layer in tqdm.tqdm([6,5,4,3,2,1,0]):
...
# use pickle to load in backup_count_storage


# with open('backup_count_storage.pickle', 'rb') as handle:
#     backup_count_storage = pickle.load(handle)

with open('THE_DANG_STORAGE_MY_GOODNESS.pickle', 'wb') as handle:
    print("DONE")
    pickle.dump(backup_count_storage, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
file_path = 'backup_count_storage.pickle'
